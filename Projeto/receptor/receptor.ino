...
String receptorNRZ_L() {
  ...
  // Enquanto não finalizar a transmissão
  while (letraBin != "11111111" && letraBin != "00000000") {

    letraBin = "";
    letra = "";

    // Lê os bits que estão chegando e os armazena em letraBin
    for (int i = 0; i < 8; i++) {
    ...
    // Palavra vai se incrementando com as letras contanto que não seja o final da trasnmissão
    if (letraBin != "11111111" && letraBin != "00000000") {
      // letraBin é traduzida para a tabela Ascii
...
String receptorNRZ_I() {
  ...
  for (int i = 0; i < (((palavraBin.length() - 8) / 8)); i++) {
    ...
    for (int j = 0; j < 8; j++) {
      if (j == 0) {
        letraBin = "0";
      } else if (sinalBin[j] != sinalBin[j - 1]) {  // Decodificação mais importante do nrzi. Onde há alteração de estado, recebe 1
        letraBin = letraBin + "1";
      } else {
        letraBin = letraBin + "0";  // Não há alteração de estado, recebe 0
      }
    }
    if(i != ((palavraBin.length() - 8) / 8)){
      ...
      receivedCRC = letraBin;
      letra = converterTabelaAscii(letraBin);  // Letra em binário é transformada em texto ascii
      palavra = palavra + letra; 
    }  
  }
...
void loop() {  // Continuará lendo (analogread) até for sinalizado com o led aceso para processar o sinal
  valorLdr = analogRead(ldr);
  Serial.print("Valor lido pelo LDR = ");
  Serial.println(valorLdr);
  delay(500);

  if (valorLdr > 500) {  // Sinal alto = começar a receber
    for (int i = 0; i < 3; i++) {
      soma += valorLdr;
      valorLdr = analogRead(ldr);
      delay(500);
